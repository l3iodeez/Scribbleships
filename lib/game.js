...
Asteroids.Game = function (canvasEl) {
...
Asteroids.Game.prototype.addAsteroids = function () {
  ...
  while (asteroids.length < this.NUM_ASTEROIDS) {
    asteroids.push(new Asteroids.Asteroid(this.randomPosition(), Asteroids.randomVec(5), Math.floor(Math.random() * 35) + 10, this));
  }
  return asteroids;
};
...
Asteroids.Game.prototype.draw = function (ctx, img) {
  ...
  // ctx.drawImage(img, 0, 0); // DRAW BG IMAGE
