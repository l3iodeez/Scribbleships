...
Asteroids.GameView.prototype.renderMenu = function (context) {
  ...
  if (this.status === "paused") {
    ...
    playButton = resume;
     ...
     _func = function (evt) {
      ...
      evt.stopPropagation();
      this.resetGame();
      start.removeEventListener('click', _func, true);
    }.bind(this);
    ...
    _func = function (evt) {
      if (evt.keyCode === 80 || evt.keyCode === 112) {
        this.resumeGame();
        document.removeEventListener('keypress', _func, false);
      }
    }.bind(this);
  ...
  playButton.addEventListener('click',  _func, true);
...
Asteroids.GameView.prototype.hideMenu = function () {
  var menuContainer = document.getElementById("menu");
  menuContainer.className = "menu hidden";
};
